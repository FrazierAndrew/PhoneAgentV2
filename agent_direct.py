"""
Direct Agent - Joins rooms immediately without waiting for dispatch
"""
import asyncio
import logging
...
# Simple HTTP server to receive join requests
...
async def handle_join_room(request):
    ...
    if not room_name:
        return web.json_response({"error": "roomName required"}, status=400)
    
    logger.info(f"📞 Received request to join room: {room_name}")
    
    # Start agent in background
    asyncio.create_task(run_agent_in_room(room_name))
    
    return web.json_response({"success": True, "room": room_name})


async def start_http_server():
    """Start HTTP server for join requests"""
    app = web.Application()
    app.router.add_post('/join-room', handle_join_room)
    ...
    logger.info("🌐 Agent HTTP server listening on port 5001")
